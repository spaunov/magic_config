// NOTE: This is almost an exact copy of the json equivalent.
...
TEST_F(JsonStringConverter, convert_string)
{
   ...
   this->check_json = [&](const Config& item) {
      EXPECT_TRUE(item.IsScalar());
      EXPECT_EQ(item.as<std::string>(), EXPECTED);
   };
...
TYPED_TEST(JsonCharConverter, convert_char)
{
   ...
   using Target = typename TestFixture::Target;

   this->check_json = [&](const Config& item) {
      EXPECT_TRUE(item.IsScalar());
      EXPECT_EQ(item.as<Target>(), EXPECTED);
   };
...
TYPED_TEST(JsonIntConverter, convert_ints)
{
   ...
   this->check_json = [&](const Config& item) {
      EXPECT_TRUE(item.IsScalar());
      EXPECT_EQ(item.as<int>(), EXPECTED);
   };
...
TYPED_TEST(JsonFloatConverter, convert_floating_points)
{
   ...
   this->check_json = [&](const Config& item) {
      EXPECT_TRUE(item.IsScalar());
      EXPECT_EQ(item.as<double>(), EXPECTED);
   };
