...
Target convert(const Config& node)
{
   using Traits = magic_config::yaml::YamlTraits;
   return magic_config::magic::detail::convert<Traits, Target>(node);
}
...
template <typename Container>
void testConvert(Config& global, const char* objName,
...
template <typename Target>
Target extract(const Config& json)
{
    try {
...
struct ConfigTest : ConfigBase
                  , magic_config::yaml::YamlConfig<ConfigTest>
{
    std::list<int>             list;
...
struct ComplexConfig : public magic_config::yaml::YamlConfig<ComplexConfig>
{
    ConfigTest  complex;

    bool verify_config(const Config& json) {
...
TEST(YamlConfig, breathing)
{
    std::ifstream t("test_script.json");
    std::string jsonStr((std::istreambuf_iterator<char>(t)),
                        std::istreambuf_iterator<char>());

    Config json = YAML::Load(jsonStr);
...
TEST(YamlConfig, missingConfig_required)
{
   std::string jsonDoc =
      R"({"conf": {
      "complex": {
...
TEST(YamlConfig, missingConfig_optional)
{
   std::string jsonDoc =
      R"({"conf": {
      "complex": {
...
TEST(YamlConfig, property_action)
{
   const size_t expectedDictSize = 2;
   g_properyActionInvoked = false;

   struct TestObj : public magic_config::yaml::YamlConfig<TestObj> {
      std::map<std::string, int> dict;
   ...
   struct TestObj : public magic_config::yaml::YamlConfig<TestObj> {
      ...
      static void defineConfigMapping() {
         TestObj::assign("power", &TestObj::outputPower)
             .required()
             .range(0, 8, false); // the range is exclsive, i.e. (0, 8)
      }
   };
